...
// expectLoadBalancerAddress to be assigned. If the address is not assigned, cross-cluster traffic will not be sent since Istio doesn't know where to send it for the remote cluster(s).
func expectLoadBalancerAddress(ctx context.Context, k kubectl.Kubectl, cl client.Client, name string) {
	Expect(common.GetSVCLoadBalancerAddress(ctx, cl, controlPlaneNamespace, name)).
