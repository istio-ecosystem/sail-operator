...
// trafficStats tracks continuous traffic statistics
...
var _ = Describe("Migration Traffic Recovery Validation", Ordered,
	Label("migration", "migration-traffic", "slow"), func() {
		// Tests migration resilience from sidecar to ambient mode with continuous traffic.
		// Uses a stable external client (sidecar mode, never migrates) to send continuous traffic
		// to a server that migrates from sidecar → ambient while traffic flows.
		// Validates:
		//   1. Limited disruption during pod restart (≤10 503 errors - brief, acceptable)
		//   2. Traffic RECOVERS after disruption (≥9/10 consecutive successes post-migration)
		//   3. Cross-mode communication works (sidecar client → ambient server)
		...
		Context(fmt.Sprintf("Migration testing with Istio %s", version.Version), func() {
			...
			Context("Namespace Mode Switching During Traffic Flow", Ordered,
				Label("migration-continuous-traffic"), func() {
					BeforeAll(func(ctx SpecContext) {
					...
					When("server workloads are deployed in sidecar mode", func() {
						...
						It("verifies initial traffic works from stable client", func(ctx SpecContext) {
							...
							Eventually(func(g Gomega) {
								...
								cmd := fmt.Sprintf("curl -s -o /dev/null -w '%%{http_code}' httpbin.%s.svc.cluster.local:8000/get --max-time 5", workloadNamespace)
								_, err := k.WithNamespace(stableClientNamespace).Exec(clientPod, "sleep", cmd)
					...
					When("namespace is switched to ambient mode during traffic", func() {
						...
						It("starts continuous traffic from stable client", func(ctx SpecContext) {
							...
							stats, cancelFunc = startContinuousTraffic(context.Background(), stableClientNamespace, clientPod, targetService, 100*time.Millisecond, nil)
    ...
    name: %s`, version.Name, ztunnelNamespace, istioName)
						...
						It("verifies traffic recovers after brief disruption during migration", func(ctx SpecContext) {
							...
							for _, err := range errors {
								if err == "unexpected status code: 503" {
									count503++
								}
							}
							...
							if count503 > 0 {
								...
								shown := 0
								for _, err := range errors {
									if err == "unexpected status code: 503" && shown < 3 {
							...
							for i := 0; i < 10; i++ {
								cmd := fmt.Sprintf("curl -s -o /dev/null -w '%%{http_code}' %s --max-time 5", targetService)
								output, err := k.WithNamespace(stableClientNamespace).Exec(clientPod, "sleep", cmd)

								if err == nil && output == "200" {
									consecutiveSuccesses++
								} else {
									recoveryFailures++
									GinkgoWriter.Printf("  [%d] Recovery check failed: %v (status: %s)\n", i+1, err, output)
								}
								time.Sleep(200 * time.Millisecond)
							}

							GinkgoWriter.Printf("Recovery verification: %d/10 consecutive requests succeeded\n", consecutiveSuccesses)

							// Assert: After migration completes, traffic should be stable (≥9/10 success)
							Expect(consecutiveSuccesses).To(BeNumerically(">=", 9),
								"Traffic should be stable after recovery (≥9/10 success), got %d/10", consecutiveSuccesses)
							...
							GinkgoWriter.Printf("Recovery stability: %d/10 consecutive successes (≥9 required)\n", consecutiveSuccesses)
							GinkgoWriter.Printf("========================================\n")
						...
						It("verifies cross-mode communication works", func(ctx SpecContext) {
							...
							Eventually(func(g Gomega) {
								...
								output, err := k.WithNamespace(stableClientNamespace).Exec(clientPod, "sleep", cmd)
								g.Expect(err).NotTo(HaveOccurred())
								g.Expect(output).To(Equal("200"), "Expected HTTP 200 from httpbin")
							}).Should(Succeed())
...
func (ts *trafficStats) recordSuccess() {
	ts.totalRequests.Add(1)
	ts.successRequests.Add(1)
}

func (ts *trafficStats) recordFailure(errMsg string) {
	ts.totalRequests.Add(1)
...
func (ts *trafficStats) getStats() (total, success, failed int64, errors []string) {
	ts.mu.Lock()
	defer ts.mu.Unlock()
	return ts.totalRequests.Load(), ts.successRequests.Load(), ts.failedRequests.Load(), append([]string{}, ts.errors...)
}

// startContinuousTraffic starts sending continuous traffic in the background
// If stats is nil, creates a new stats object. Otherwise reuses the existing one.
// Returns the stats object and a stop function that should be called to stop the traffic
func startContinuousTraffic(
	ctx context.Context, namespace, clientPod, targetService string,
	interval time.Duration, existingStats *trafficStats,
) (*trafficStats, context.CancelFunc) {
	stats := existingStats
	if stats == nil {
		stats = &trafficStats{}
	}
	...
	go func() {
		defer GinkgoRecover()
		GinkgoWriter.Printf("[Traffic] Starting continuous traffic from %s to %s (interval: %v)\n", clientPod, targetService, interval)
			...
			go func() {
				...
				defer func() {
					if r := recover(); r != nil {
						...
						GinkgoWriter.Printf("[Traffic] %s\n", errMsg)
					}
				}()

				cmd := fmt.Sprintf("curl -s -o /dev/null -w '%%{http_code}' %s --max-time 2", targetService)
				output, err := k.WithNamespace(namespace).Exec(clientPod, "sleep", cmd)
				if err != nil {
		...
		for {
			...
			case <-trafficCtx.Done():
				GinkgoWriter.Printf("[Traffic] Stopping continuous traffic (context cancelled)\n")
				return
			case <-ticker.C:
				sendRequest()
			}
