...
var sailCRDs = []string{
	...
	"istiorevisions.sail-operator.io",
	"istios.sail-operator.io",
...
var _ = Describe("Operator", Ordered, func() {
	...
	Describe("installation", func() {
		...
		Specify("istio crd is present", func(ctx SpecContext) {
			...
			Eventually(cl.Get).WithArguments(ctx, kube.Key("istios.sail-operator.io"), &apiextensionsv1.CustomResourceDefinition{}).
