...
```bash { name=wait-operator tag=example}
...
kubectl apply -n sample -f https://raw.githubusercontent.com/istio/istio/release-1.23/samples/bookinfo/platform/kube/bookinfo.yaml
```
```bash { name=wait-sample-app tag=example}
