...
for file in "$DOCS_DIR"/*; do
  if [ -d "$file" ]; then
    # If it's a directory, check the files inside it.
    for subfile in "$file"/*; do
      if [[ "$subfile" == *.md && ! " ${EXCLUDE_FILES[@]} " =~ $(basename "$subfile") ]]; then
        # Only add if the file contains the pattern 'bash { name='
        if grep -q "bash { name=" "$subfile"; then
          FILES_TO_COPY+=("$subfile")
        fi
      fi
    done
  else
    # If it's a file, check if it matches.
    if [[ "$file" == *.md && ! " ${EXCLUDE_FILES[@]} " =~ $(basename "$file") ]]; then
