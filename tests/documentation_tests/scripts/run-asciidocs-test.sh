...
function create_test_files() {
    # Get all the .adoc files in the DOCS_DIR
    FILES_TO_CHECK=()
    for file in "$DOCS_DIR"/*.adoc; do
        FILES_TO_CHECK+=("$file")
    done
    ...
    for test_kv in "${TEST_KV[@]}"; do
        ...
        # write the content of the test by extracting all the lines between the ifdef and endif for all the corresponding conditional structure
        awk "/ifdef::$test_name\\[\\]/,/endif::\\[\\]/" "$file" | grep -vE "^(ifdef::|endif::)" >> "$test_file"
