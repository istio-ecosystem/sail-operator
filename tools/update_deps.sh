...
if [[ "${UPDATE_BRANCH}" == "master" ]]; then
  "$SED_CMD" -i -e "s|\(registry.istio.io/testing/build-tools\):master.*|\1:$NEW_IMAGE_MASTER|" .github/workflows/update-deps.yaml .github/workflows/update-eol-versions.yaml .github/workflows/sync-changelog.yaml
