...
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1alpha1.IstioRevisionTag{}, revisionTagHandler, builder.WithPredicates(ignoreStatusChange())).
	...
	if ok {
		if tag.Spec.TargetRef.Kind == v1alpha1.IstioRevisionKind {
			...
			if err := r.Client.Get(ctx, types.NamespacedName{Name: tag.Spec.TargetRef.Name}, i); err != nil {
				log.Error(err, "failed to get Istio resource referenced by IstioRevisionTag "+tag.Name)
				return nil
			}
