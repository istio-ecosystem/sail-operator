...
func (r *Reconciler) validate(ctx context.Context, tag *v1alpha1.IstioRevisionTag) error {
	...
	// TODO: validate that no revision exists with this name
	// TODO: validate that the object that is referenced exists
	return nil
}
...
func (r *Reconciler) getIstioRevision(ctx context.Context, ref v1alpha1.IstioRevisionTagTargetReference) (*v1alpha1.IstioRevision, error) {
	if ref.Kind == v1alpha1.IstioRevisionKind {
		...
		err := r.Client.Get(ctx, types.NamespacedName{Name: ref.Name}, &rev)
		if err != nil {
...
func (r *Reconciler) installHelmCharts(ctx context.Context, tag *v1alpha1.IstioRevisionTag, rev *v1alpha1.IstioRevision) error {
	...
	//values.Set("pilot.revisionTags", []string{tag.Name})
...
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	logger := mgr.GetLogger().WithName("ctrlr").WithName("istiorevtag")

	// mainObjectHandler handles the IstioRevisionTag watch events
	mainObjectHandler := enqueuelogger.WrapIfNecessary(v1alpha1.IstioRevisionTagKind, logger, &handler.EnqueueRequestForObject{})

	// ownedResourceHandler handles resources that are owned by the IstioRevisionTag CR
	ownedResourceHandler := enqueuelogger.WrapIfNecessary(v1alpha1.IstioRevisionTagKind, logger,
		handler.EnqueueRequestForOwner(r.Scheme, r.RESTMapper(), &v1alpha1.IstioRevisionTag{}, handler.OnlyControllerOwner()))
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1alpha1.Istio{}, ownedResourceHandler).
...
func (r *Reconciler) determineStatus(ctx context.Context, tag *v1alpha1.IstioRevisionTag, rev *v1alpha1.IstioRevision, reconcileErr error) (v1alpha1.IstioRevisionTagStatus, error) {
...
func deriveState(reconciledCondition v1alpha1.IstioRevisionTagCondition) v1alpha1.IstioRevisionTagConditionReason {
	if reconciledCondition.Status != metav1.ConditionTrue {
		return reconciledCondition.Reason
	}
	return v1alpha1.IstioRevisionTagReasonReconcileError
}
...
func (r *Reconciler) isRevisionTagReferencedByWorkloads(ctx context.Context, tag *v1alpha1.IstioRevisionTag) (bool, error) {
	...
	for _, pod := range podList.Items {
		if ns, found := nsMap[pod.Namespace]; found && podReferencesRevisionTag(pod, ns, tag) {
...
func podReferencesRevisionTag(pod corev1.Pod, ns corev1.Namespace, tag *v1alpha1.IstioRevisionTag) bool {
	return tag.Name == getReferencedRevisionFromPod(pod.GetLabels(), pod.GetAnnotations(), ns.GetLabels())
}
