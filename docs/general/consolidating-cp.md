...
It's possible to create multiple control planes in a single [cluster](https://istio.io/latest/docs/setup/install/multiple-controlplanes/). In case it's no longer needed to use multiple control planes, it might be useful to consolidate them to only one. This documentation shows how to achieve that without any traffic disruption.

## Prerequisites
By default, each control plane is using a different root certificate which prevents to move workloads from one mesh to another without breaking mTLS. In this procedure, we expect that each control plane is using an intermediate certificate issued by the same root CA. How to achieve this without any traffic disruption, see [Plug in CA Certificates](plugin-ca.md).
...
1. Even though the communication works, proxies are still connected to different control planes:
    ...
    ```
1. Update injection labels to connect proxies to first control plane:
    ```bash
...
1. Restart workloads:
    ...
    ```
1. Verify that all proxies are connected to first control plane:
    ```bash
    istioctl ps -i istio-system-a
    NAME                                   CLUSTER        CDS              LDS              EDS              RDS              ECDS        ISTIOD                                      VERSION
