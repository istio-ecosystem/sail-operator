...
:istio_latest_version: 1.31.0-beta.1
:istio_latest_version_revision_format: 1-31-0-beta-1
:istio_latest_tag: v1.31.0-beta-latest
:istio_release_name: release-1.31
:istio_latest_minus_one_version: 1.30.3
:istio_latest_minus_one_version_revision_format: 1-30-3
...
**Note:** Istio v1.27 is the minimum supported version for this deployment model.
...
. Create `istio-cni` namespace on `cluster1`
+
----
kubectl get ns istio-cni --context "${CTX_CLUSTER1}" || kubectl create namespace istio-cni --context "${CTX_CLUSTER1}"
----

. Create an `IstioCNI` resource on `cluster1`
+
----
kubectl apply --context "${CTX_CLUSTER1}" -f - <<EOF
apiVersion: sailoperator.io/v1
...
. Create `istio-cni` namespace on `cluster2`
+
----
kubectl get ns istio-cni --context "${CTX_CLUSTER2}" || kubectl create namespace istio-cni --context "${CTX_CLUSTER2}"
----

. Create an `IstioCNI` resource on `cluster2`
+
----
kubectl apply --context "${CTX_CLUSTER2}" -f - <<EOF
apiVersion: sailoperator.io/v1
...
+
----
kubectl delete ztunnels default --context="${CTX_CLUSTER1}"
kubectl delete istiocnis default --context="${CTX_CLUSTER1}"
kubectl delete istios default --context="${CTX_CLUSTER1}"
kubectl delete ns istio-system --context="${CTX_CLUSTER1}"
kubectl delete ns sample --context="${CTX_CLUSTER1}"
kubectl delete ns ztunnel --context="${CTX_CLUSTER1}"
kubectl delete ns istio-cni --context="${CTX_CLUSTER1}"
kubectl delete ztunnels default --context="${CTX_CLUSTER2}"
kubectl delete istiocnis default --context="${CTX_CLUSTER2}"
kubectl delete istios default --context="${CTX_CLUSTER2}"
kubectl delete ns istio-system --context="${CTX_CLUSTER2}"
kubectl delete ns sample --context="${CTX_CLUSTER2}"
kubectl delete ns ztunnel --context="${CTX_CLUSTER2}"
kubectl delete ns istio-cni --context="${CTX_CLUSTER2}"
----
...
[WARNING]
====
⚠️ This tutorial assumes your cloud provider allocates an IP address for
the Istio IngressGateway LoadBalancer.
If your provider uses a DNS name (e.g., AWS ELB),
you must ensure the `tls-webhook` port is exposed
correctly on the provider's load balancer.

The Challenge: Port Mapping
In an external-istiod installation, the remote cluster uses a local service to reach the external Istio instance via a mutating webhook.

The webhook configuration always expects port 443:
[source,yaml]
----
clientConfig:
  service:
    name: istiod-external-istiod
    namespace: external-istiod
    path: /inject/cluster/cluster2/net/network1
    port: 443 # Hardcoded to 443
----
However, the `tls-webhook` is typically exposed on port `15017` at the Ingress Gateway.
To bridge this, a Kubernetes Service and Endpoint are used to map port `443` to `15017`.

The issue with DNS-based Load Balancers:

While `Service` + `Endpoint` mapping works perfectly with IPs,
it fails when using a DNS-based LoadBalancer.
In these cases, the service must be of type `ExternalName`, which does not
support `targetPort` mapping.
Consequently, the remote cluster cannot reach the external webhook because the port mismatch remains unresolved.

The Solution

If your provider allocates DNS names, you must bypass the mapping requirement
by exposing the webhook directly on port `443` at the Ingress Gateway.
Patch the istio-ingressgateway service to change
the `tls-webhook` port from `15017` to `443`
[source,bash]
----
kubectl patch svc istio-ingressgateway \
  -n istio-system \
  --type='json' -p='[{"op": "replace", "path": "/spec/ports/filter_by_name_tls-webhook/port", "value":443}]'
----
====
