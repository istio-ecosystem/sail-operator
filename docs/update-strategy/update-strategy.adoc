...
- <<update-strategy>>
  ...
  - <<revisionbased>>
    - <<revisionbased-ambient-mode-considerations>>
...
*Note:* Both strategies work with sidecar and ambient modes.

**For ambient mode updates:**

* Components update order: Control Plane → IstioCNI → ZTunnel
* No pod restart needed (workloads automatically connect to new control plane)
* Use InPlace strategy (recommended) - RevisionBased has limitations in ambient mode
...
With InPlace updates in ambient mode, all components update directly and workloads immediately connect to the new control plane. IstioCNI and ZTunnel use rolling updates. Brief traffic interruption may occur during control plane updates. Running multiple istiod replicas helps minimize this risk but doesn't eliminate it. For details, see the https://istio.io/latest/docs/ambient/upgrade/helm/#understanding-ambient-mode-upgrades[Istio ambient mode upgrade documentation].
...
[[revisionbased-ambient-mode-considerations]]
=== RevisionBased Strategy for Ambient Mode
...
**Recommendation:** Use InPlace updates for ambient mode. It's simpler and more reliable. Use RevisionBased only if you have specific requirements for canary upgrades.

Unlike sidecar mode, ambient workloads don't use namespace labels like `istio.io/rev` for version selection. Instead, ambient workloads automatically connect to the active control plane revision.

IstioCNI works with multiple control plane versions during migration. However, only one ztunnel runs cluster-wide, requiring careful coordination.

See <<updating-ambient-components>> for IstioCNI and ZTunnel update procedures.
...
* Existing connections may reset after a grace period during node updates
