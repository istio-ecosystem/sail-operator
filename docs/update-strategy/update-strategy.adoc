...
IMPORTANT: **Only the InPlace strategy is recommended for ambient mode.** RevisionBased updates require manual effort to keep Istio and ZTunnel revisions in sync, and only one ztunnel instance can run in the cluster at a time, making canary-style upgrades impractical.
