...
Ztunnel operates only at Layer 4 of the OSI Model using TCP so it's not possible to pass state over to the other process. This has significant implications for the long-lived TCP connections on the upgraded node. The Ztunnel runs as a DaemonSet — a per-node proxy — meaning that ztunnel upgrades affect, at minimum, an entire node at a time.
...
. At this point, we have both Ztunnels listening. New connections will be assigned to either one.
...
. Immediately upon starting a drain, the old ztunnel will close its listeners. Now only the new ztunnel is listening. Critically, at all times there was at least one ztunnel listening.
...
. drain a node
. delete old ZTunnel pod and wait for new one to be started
