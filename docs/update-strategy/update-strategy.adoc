...
IMPORTANT: **In ambient mode, the control plane supports both the `InPlace` and `RevisionBased` strategies.** The ambient data-plane components (`ZTunnel` and `IstioCNI`) are deployed as node-level shared DaemonSets and are always updated in place; they are not deployed per revision and cannot be canaried side by side. With `RevisionBased`, multiple `istiod` revisions can coexist while sharing the single ztunnel, which must remain within its supported version skew (ztunnel 1.x is compatible with control plane 1.x and 1.x+1).
...
This section covers updating IstioCNI, ZTunnel, and Waypoint proxies after a control plane update. These ambient data-plane components are always updated in place, regardless of the control plane's update strategy.
