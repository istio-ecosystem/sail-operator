...
* link:update-strategy/update-strategy.adoc#updating-ambient-components[Updating Ambient Mode Components]
...
[NOTE]
====
The cluster-wide `istiod-default-validator` `ValidatingWebhookConfiguration` is only installed if either the revision or the revision tag is named `default`. Every revision always gets its own `istio-validator-<revision>-istio-system` webhook, but that one has an `objectSelector` on `istio.io/rev`, so it only validates resources that carry the matching revision label. The `istiod-default-validator` webhook has no such selector and is therefore the one that validates Istio resources created without the `istio.io/rev` label.

So if you use a named revision (for example `prod`) and don't create an `IstioRevisionTag` named `default` pointing to it, Istio resources without the `istio.io/rev` label are not validated at admission time.

This is a bug and will be fixed in a future release; the `istiod-default-validator` webhook should be installed regardless of the revision or revision tag name.
====
...
The Sail Operator API reference documentation can be found link:api-reference/sailoperator.io.md#api-reference[here].
