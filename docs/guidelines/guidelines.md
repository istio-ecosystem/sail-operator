...
1. Add into the example or the new example in the docs the next patterns in exact order into each code block `bash` that will be running the example:
...
- Validation steps are always needed, to add a validation step you can comment directly the entire code block with the bash command that you want to run. This validation steps need to use the same tag as the example where they are going to run and should be prefixed with `validation-validation-description`. For example:
```md
