...
ifdef::ambient-waypoint-prerequisites[]
kubectl get crd gateways.gateway.networking.k8s.io &> /dev/null || \
  { kubectl apply -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.2.0/standard-install.yaml; }
# Wait for the Gateway API CRDs to be established
with_retries kubectl get crd gateways.gateway.networking.k8s.io
endif::[]
...
ifdef::ambient-waypoint-deploy[]
# Ensure bookinfo namespace exists and is in ambient mode
kubectl create ns bookinfo --dry-run=client -o yaml | kubectl apply -f -
...
spec:
  ...
  - name: mesh
    ...
    protocol: HBONE
EOF
# Wait for the waypoint deployment to be created
with_retries kubectl get deployment waypoint -n bookinfo
# Wait for waypoint pods to be ready
with_retries wait_pods_by_label "bookinfo" "gateway.networking.k8s.io/gateway-name=waypoint"
endif::[]
...
ifdef::ambient-waypoint-deploy[]
kubectl label ns bookinfo istio.io/use-waypoint=waypoint --overwrite
# Verify the label was applied
with_retries kubectl get ns bookinfo -o jsonpath='{.metadata.labels.istio\.io/use-waypoint}' | grep -q "waypoint"
endif::[]
...
ifdef::ambient-waypoint-crossns-deploy[]
# Create the foo namespace
kubectl create ns foo --dry-run=client -o yaml | kubectl apply -f -
# Deploy the cross-namespace waypoint in foo namespace
kubectl apply -f - <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: Gateway
metadata:
...
EOF
# Wait for the waypoint deployment to be created
with_retries kubectl get deployment waypoint-foo -n foo
# Wait for waypoint pods to be ready
with_retries wait_pods_by_label "foo" "gateway.networking.k8s.io/gateway-name=waypoint-foo"
# Apply labels to bookinfo namespace
...
with_retries kubectl get ns bookinfo -o jsonpath='{.metadata.labels.istio\.io/use-waypoint-namespace}' | grep -q "foo"
with_retries kubectl get ns bookinfo -o jsonpath='{.metadata.labels.istio\.io/use-waypoint}' | grep -q "waypoint-foo"
endif::[]
...
Waypoint proxies automatically update when the control plane is updated. The istiod control plane reconciles the waypoint deployment and updates it to the new version.

**InPlace Strategy:**

When using InPlace updates, waypoint proxies automatically transition to the new control plane version.

1. Verify waypoint proxy pods are running:
+
...
----
+
ifdef::ambient-waypoint-update-inplace[]
# Verify waypoint proxy pods are running
echo "Verifying waypoint proxy pods are running..."
kubectl get pods -n bookinfo -l gateway.networking.k8s.io/gateway-name=waypoint
with_retries wait_pods_by_label "bookinfo" "gateway.networking.k8s.io/gateway-name=waypoint"
endif::[]
...
----
+
The `SYNCED` status indicates the waypoint is receiving configuration from the control plane. The version column shows the updated Istio version.
+
ifdef::ambient-waypoint-update-inplace[]
# Confirm the waypoint proxy is connected to the control plane
echo "Confirming waypoint proxy connection to control plane..."
istioctl proxy-status | grep waypoint || echo "Waypoint proxy status checked"
endif::[]

**RevisionBased Strategy:**

When using RevisionBased updates, waypoint proxies automatically connect to the active control plane revision. The istiod control plane reconciles the waypoint deployment and updates it to the new version.
...
----
+
ifdef::ambient-waypoint-update-revisionbased[]
# Verify waypoint proxy pods are running after revision update
echo "Verifying waypoint proxy pods are running after revision update..."
kubectl get pods -n bookinfo -l gateway.networking.k8s.io/gateway-name=waypoint
with_retries wait_pods_by_label "bookinfo" "gateway.networking.k8s.io/gateway-name=waypoint"
endif::[]
...
----
+
The output shows the waypoint proxy connected to the revision-specific istiod pod (note the revision `1-26-4` in the pod name) and running the updated version.
+
ifdef::ambient-waypoint-update-revisionbased[]
# Confirm the waypoint proxy is connected to the new control plane revision
echo "Confirming waypoint proxy connection to control plane revision..."
istioctl proxy-status | grep waypoint || echo "Waypoint proxy status checked"
endif::[]
...
After updating waypoint proxies, verify Layer 7 features work correctly.

**Traffic Routing:**

Test that HTTPRoute rules are enforced. See <<traffic-routing,Traffic Routing>> for setup details with a 90/10 traffic split.

1. Test traffic distribution by sending requests and checking which service version handles them:
+
...
----
+
The output should match your HTTPRoute configuration. With a 90/10 split, expect about 9 requests to reviews-v1 and 1 to reviews-v2.
+
ifdef::ambient-waypoint-verify-l7[]
# Test traffic distribution
echo "Testing traffic distribution with HTTPRoute..."
for i in {1..10}; do
  kubectl exec "$(kubectl get pod -l app=productpage -n bookinfo -o jsonpath='{.items[0].metadata.name}')" -c istio-proxy -n bookinfo -- curl -s http://reviews:9080/reviews/0 | grep -o "reviews-v[0-9]" || true
done
endif::[]

**Authorization Policies:**

Test that L7 authorization policies are enforced. The `productpage-waypoint` AuthorizationPolicy (see <<security-authorization,Security Authorization>>) allows only the curl service to send GET requests to productpage.

1. Test denied access - Services not in the allow list should be denied:
+
[source,bash,subs="attributes+",name="ambient-waypoint-verify-l7"]
----
...
----
+
The ratings service is denied because it's not in the allow list.
+
ifdef::ambient-waypoint-verify-l7[]
# Test denied access from ratings service
echo "Testing denied access from ratings service..."
kubectl exec "$(kubectl get pod -l app=ratings -n bookinfo -o jsonpath='{.items[0].metadata.name}')" -c ratings -n bookinfo -- curl -sS productpage:9080/productpage | grep -q "RBAC: access denied" && echo "Access correctly denied" || echo "Expected denial not found"
endif::[]

2. Test allowed access - The curl service should access productpage successfully:
+
[source,bash,subs="attributes+",name="ambient-waypoint-verify-l7"]
----
...
----
+
The request succeeds because the curl service matches the policy rules.
+
ifdef::ambient-waypoint-verify-l7[]
# Test allowed access from curl service
echo "Testing allowed access from curl service..."
kubectl exec "$(kubectl get pod -l app=curl -n default -o jsonpath='{.items[0].metadata.name}')" -c curl -n default -- curl -sS http://productpage.bookinfo:9080/productpage | grep -q "<title>Simple Bookstore App</title>" && echo "Access correctly allowed" || echo "Expected access not found"
endif::[]

[[cross-namespace-waypoint-updates]]
=== Cross-namespace Waypoint Updates

For cross-namespace waypoints, verify the namespace labels are correct.
...
----
+
ifdef::ambient-waypoint-crossns-update[]
...
ifdef::ambient-waypoint-l7-features[]
kubectl apply -f - <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
with_retries kubectl get httproute reviews -n bookinfo
endif::[]
...
ifdef::ambient-waypoint-l7-features[]
# Create default namespace and deploy curl service for testing
kubectl create ns default --dry-run=client -o yaml | kubectl apply -f -
kubectl label namespace default istio.io/dataplane-mode=ambient --overwrite
kubectl apply -n default -f https://raw.githubusercontent.com/istio/istio/{istio_release_name}/samples/curl/curl.yaml
# Wait for curl pod to be ready
with_retries wait_pods_by_label "default" "app=curl"
...
spec:
  ...
  - from:
    ...
    - operation:
        methods: ["GET"]
EOF
# Verify AuthorizationPolicy was created
with_retries kubectl get authorizationpolicy productpage-waypoint -n bookinfo
endif::[]
...
ifdef::ambient-waypoint-cleanup[]
kubectl delete -n bookinfo AuthorizationPolicy productpage-waypoint --ignore-not-found=true
kubectl delete -n bookinfo HTTPRoute reviews --ignore-not-found=true
kubectl delete -n bookinfo Gateway waypoint --ignore-not-found=true
# Also delete cross-namespace waypoint if it exists
kubectl delete -n foo Gateway waypoint-foo --ignore-not-found=true
kubectl delete ns foo --ignore-not-found=true
endif::[]
...
ifdef::ambient-waypoint-cleanup[]
kubectl label namespace bookinfo istio.io/dataplane-mode- --ignore-not-found=true
kubectl label namespace bookinfo istio.io/use-waypoint- --ignore-not-found=true
kubectl label namespace bookinfo istio.io/use-waypoint-namespace- --ignore-not-found=true
kubectl label namespace default istio.io/dataplane-mode- --ignore-not-found=true
endif::[]
...
ifdef::ambient-waypoint-cleanup[]
kubectl delete -n bookinfo -f https://raw.githubusercontent.com/istio/istio/{istio_release_name}/samples/bookinfo/platform/kube/bookinfo.yaml --ignore-not-found=true
kubectl delete -n bookinfo -f https://raw.githubusercontent.com/istio/istio/{istio_release_name}/samples/bookinfo/platform/kube/bookinfo-versions.yaml --ignore-not-found=true
kubectl delete -n bookinfo -f https://raw.githubusercontent.com/istio/istio/{istio_release_name}/samples/bookinfo/gateway-api/bookinfo-gateway.yaml --ignore-not-found=true
kubectl delete -n default -f https://raw.githubusercontent.com/istio/istio/{istio_release_name}/samples/curl/curl.yaml --ignore-not-found=true
endif::[]
