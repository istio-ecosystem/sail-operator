...
IMPORTANT: The **InPlace update strategy is the recommended approach** for ambient mode deployments. While RevisionBased updates are theoretically possible, they require significant manual effort to synchronize CRDs between Istio and ZTunnel revisions and have inherent limitations (only one ztunnel instance can run cluster-wide). See link:../update-strategy/update-strategy.adoc#revisionbased-ambient-mode-considerations[RevisionBased Strategy for Ambient Mode] for details on these limitations.

The update strategy guide covers:

- InPlace strategy (recommended for ambient mode) - see link:../update-strategy/update-strategy.adoc#inplace-ambient-mode-considerations[InPlace Strategy for Ambient Mode]
- RevisionBased strategy (with limitations) - see link:../update-strategy/update-strategy.adoc#revisionbased-ambient-mode-considerations[RevisionBased Strategy for Ambient Mode]
