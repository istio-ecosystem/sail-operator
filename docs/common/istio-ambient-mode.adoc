...
+
NOTE: The `targetRef` field links the ZTunnel to the Istio control plane, so that relevant configuration values are automatically inherited.
+
[source,bash,subs="attributes+"]
----
cat <<EOF | kubectl apply -f-
...
For detailed information on updating Istio when deployed in ambient mode, see link:../update-strategy/update-strategy.adoc#update-strategy[Update Strategy].

IMPORTANT: **Only the InPlace update strategy is supported for ambient mode.** This is due to ZTunnel's cluster-wide singleton architecture - only one ztunnel instance can run in the cluster at a time, making canary-style upgrades impractical with RevisionBased updates.

The update strategy guide covers:

- InPlace strategy for ambient mode - see link:../update-strategy/update-strategy.adoc#inplace-ambient-mode-considerations[InPlace Strategy for Ambient Mode]
- Updating IstioCNI, ZTunnel, and Waypoint proxies - see link:../update-strategy/update-strategy.adoc#updating-ambient-components[Updating Ambient Mode Components]
