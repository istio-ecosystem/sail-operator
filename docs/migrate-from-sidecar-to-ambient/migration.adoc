...
install_bookinfo "{istio_release_name}"
create_bookinfo_gateway_api "{istio_release_name}"
with_retries wait_pods_ready_by_ns "bookinfo"
echo "Checking if pods have their application container and the Istio sidecar container (READY 2/2):"
kubectl get pods -n "bookinfo" -l app
endif::[]
...
|No change required
|Only enforced when a waypoint is deployed and traffic is directed through it; silently ignored in ztunnel-only deployments
...
- Primary-remote multicluster configurations (multi-primary multicluster IS supported)
...
kubectl get peerauthentication -A -o yaml | grep -B5 "mode: DISABLE" || echo "No DISABLE mode found"

# Check for EnvoyFilter resources (not supported on waypoint proxies)
kubectl get envoyfilter -A || echo "No EnvoyFilter resources found"

# Check for primary-remote multicluster config (blocker; multi-primary is OK)
kubectl get istio -A -o yaml | grep -i "remote\|multiCluster" || echo "No multicluster config found"
----
...
Verify the Istio resource is ready again:
[source,bash,subs="attributes+",name="istio-migrate-from-sidecar-to-ambient"]
----
...
kubectl get namespace istio-cni >/dev/null 2>&1 || kubectl create namespace istio-cni
...
[source,bash,subs="attributes+",name="istio-migrate-from-sidecar-to-ambient"]
----
# Create ZTunnel namespace if it doesn't exist
...
kubectl wait --for=condition=Complete --timeout=120s job/wait-for-configmap -n ztunnel 2>/dev/null || \
  for i in {1..30}; do \
    kubectl get configmap istio-ca-root-cert -n ztunnel >/dev/null 2>&1 && break || sleep 2; \
  done

# Apply ZTunnel configuration
cat <<EOF | kubectl apply -f-
apiVersion: sailoperator.io/v1
kind: ZTunnel
metadata:
...
ifdef::istio-migrate-from-sidecar-to-ambient[]
with_retries wait_pods_ready_by_ns "ztunnel"
endif::[]
...
kubectl get pod $(kubectl get pods -n bookinfo -l app=ratings -o jsonpath='{.items[0].metadata.name}') -n bookinfo -o yaml | grep -A 2 'name: PROXY_CONFIG'
----
...
Apply the Waypoint for bookinfo namespace configurations:
[source,bash,subs="attributes+",name="istio-migrate-from-sidecar-to-ambient"]
----
cat <<EOF | kubectl apply -f-
...
**Apply the bookinfo Authorization Policies:**
...
spec:
  ...
  - from:
    ...
    - operation:
        methods: ["GET", "POST"]
---
# Reviews service authorization - allows traffic from productpage, reviews, and ratings
apiVersion: security.istio.io/v1
kind: AuthorizationPolicy
metadata:
...
spec:
  ...
  - from:
    - source:
        principals: ["cluster.local/ns/bookinfo/sa/waypoint"]
---
apiVersion: security.istio.io/v1
kind: AuthorizationPolicy
metadata:
...
kubectl exec -n bookinfo $(kubectl get pods -n bookinfo -l app=ratings -o jsonpath='{.items[0].metadata.name}') -- curl -s http://reviews.bookinfo:9080/reviews/1
...
Test the traffic routing through waypoint with header (if HTTPRoute configured):

[source,bash,subs="attributes+",name="istio-migrate-from-sidecar-to-ambient"]
----
kubectl exec -n bookinfo $(kubectl get pods -n bookinfo -l app=ratings -o jsonpath='{.items[0].metadata.name}') -- curl -s -H "end-user: jason" http://reviews.bookinfo:9080/reviews/1
----
