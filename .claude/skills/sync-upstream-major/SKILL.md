...
- **构建环境自适应**：脚本自动探测 docker/podman/nerdctl——有可用的就走容器模式（make 默认），没有则 `BUILD_WITH_CONTAINER=0` 本地工具链（自动补装 yq、license-lint 到 `./bin`）。两种模式生成结果一致。
...
全部解决后 `git add <文件>` 并 `git commit -s --no-edit` 完成合并。
...
脚本自动探测构建模式，然后 `go mod tidy` → 清理 `resources/` 版本目录（保留 `resources.go` 等非版本文件，它是 release-1.30 起被跟踪的 go:embed 包）→ `make gen` → `make alauda-update-values` → 清理上游命名的 CSV 残留（`bundle/manifests/sailoperator.clusterserviceversion.yaml`）→ `git add -A`。**耗时 5~15 分钟**，Bash 调用要设大 timeout（如 900000）或后台运行；失败时 make 的报错就是现场，修复后整体重跑（幂等）。

生成后过一遍 `git diff HEAD -- chart/values.yaml`：个别 alauda 定制值可能被上游改动或重新生成覆盖，需要手动补回。
...
然后跑 `make lint` 与 `make test`（耗时较长，设大 timeout 或后台运行）。本地工具链模式下个别 lint 工具若缺失，如实记录并汇报，由用户定夺是否需要容器环境补跑。
