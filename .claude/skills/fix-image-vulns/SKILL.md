...
- resolve 会把当前检出分支记为基分支（修复分支从它切出、PR 以它为 base）。若提示 run 构建分支与当前分支不一致，用 AskUserQuestion 和用户确认基分支，需要时检出正确分支后重跑。
...
多平台镜像构建通常几十分钟，**必须后台运行**（Bash 的 `run_in_background: true`）：
...
2. 首轮扫描摘要（总数、分类计数）→ 修复清单（模块/工具链、版本变化、覆盖的 CVE、commit）→ PR 链接 → 流水线 run 链接与结果 → 回归扫描结论；
