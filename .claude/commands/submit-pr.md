...
# Push branch
git push -u "$FORK_REMOTE" "$CURRENT_BRANCH" >/dev/null || { echo "❌ Failed to push branch"; exit 1; }
...
if [ "$IS_FORK" = "true" ]; then
    ...
    EXISTING_PR_INFO=$(gh pr list --repo "$TARGET_REPO" --head "$FORK_OWNER:$CURRENT_BRANCH" --state open --json number,title,url --jq '.[0] // empty' 2>/dev/null || echo "")
else
    ...
    if [ -n "$EXISTING_PR_NUMBER" ] && [ "$EXISTING_PR_NUMBER" != "null" ]; then
        ...
        case $CHOICE in
            ...
            2)
                ...
                # Continue to regular PR creation
                ;;
            3)
                echo ""
